...
class MockDatabase:
    ...
    # Частые вопросы сгенерированы на основе анализа 1000+ реальных обращений в банке Сбер
    # за последние 3 месяца (синтетические данные, сгенерированные LLM)
    FREQUENT_QUESTIONS = [
        {
            "id": 1,
...
🔐 Если не получается - обратитесь к администратору""",
    ...
    async def search_knowledge_base(query: str) -> Dict[str, Any]:
        """Поиск в базе знаний с определением уверенности"""
        ...
        if best_match and best_score >= 2:
            ...
            return {
                ...
                "source": f"📚 База знаний Сбер | Частый вопрос #{best_match['id']}",
...
class MockLLMService:
    ...
    async def analyze_problem(user_message: str) -> Dict[str, Any]:
        ...
        if any(word in user_message_lower for word in ['доступ', 'войти', 'логин', 'пароль']):
            category = "Проблемы с доступом"
            subcategory = "Аутентификация"
        elif any(word in user_message_lower for word in ['отчет', 'формирование', 'аналитика', 'данные']):
            category = "Работа с отчетами"
            subcategory = "Формирование отчетов"
        elif any(word in user_message_lower for word in ['медленно', 'тормозит', 'зависает', 'скорость']):
            category = "Производительность"
            subcategory = "Медленная работа"
        elif any(word in user_message_lower for word in ['почта', 'email', 'письмо', 'отправка']):
            category = "Корпоративная почта"
            subcategory = "Работа с почтой"
        elif any(word in user_message_lower for word in ['платеж', 'транзакция', 'деньги', 'финанс']):
...
class MockTicketSystem:
    ...
    async def get_ticket_status(ticket_id: str) -> Optional[Dict[str, Any]]:
        """Получить статус тикета с автоматическим обновлением"""
        ...
        # Имитация изменения статуса со временем
        current_status = ticket["status"]
        
        if current_status == "created" and time_diff.total_seconds() > 60:  # 1 минута
        ...
        elif current_status == "in_progress" and time_diff.total_seconds() > 300:  # 5 минут
        ...
        elif current_status in ["awaiting_info", "awaiting_confirmation"] and time_diff.total_seconds() > 600:  # 10 минут
...
def get_ticket_actions_keyboard(ticket_id: str):
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text="💬 Добавить комментарий", callback_data=f"comment_{ticket_id}")],
...
async def show_faq_menu(message: types.Message):
    """Показать меню с частыми вопросами (inline-кнопки в чате)"""
    faq_items = MockDatabase.FREQUENT_QUESTIONS
    
    # Формируем текст с описанием
    faq_text = """<b>📋 Частые вопросы пользователей</b>

<i>Выберите интересующий вас вопрос из списка ниже:</i>

<b>Самые популярные проблемы:</b>"""
    
    for faq in faq_items:
        faq_text += f"\n• {faq['question']} ({faq['frequency']})"
    
    faq_text += "\n\n💡 <i>Нажмите на кнопку с вопросом, чтобы увидеть решение</i>"
...
• Приоритет: {faq_item['priority']}
• Встречаемость: {faq_item['frequency']}
...
async def check_ticket_status(message: types.Message):
    ...
    # Создаем mock-тикет для демонстрации, если у пользователя нет активных тикетов
    user_tickets = await MockTicketSystem.get_user_tickets(message.from_user.id)
    
    if not user_tickets:
        # Создаем демонстрационный тикет
        ticket_id = f"SBER-{datetime.now().strftime('%y%m%d')}-{random.randint(1000, 9999)}"
        
        # Сохраняем mock тикет
        MockTicketSystem._ticket_statuses[ticket_id] = {
            "status": "in_progress",
            "user_id": message.from_user.id,
            "problem": "Пример: Не работает доступ к системе отчетности",
            "category": "Проблемы с доступом",
            "critical_level": "medium",
            "priority": "Средний",
            ...
            "updates": [
                ...
                },
                {
                    ...
                    "status": "in_progress",
                    "message": "Проводится диагностика проблемы"
                }
            ]
        }
    ...
    if not ticket:
        await message.answer(
            ...
            "Попробуйте проверить статус позже или создайте новое обращение.",
            parse_mode="HTML",
            reply_markup=get_main_keyboard()
        )
        return
...
✅ <b>Это решение помогло решить вашу проблему?</b>"""
    ...
    # Принимаем решение на основе уверенности
...
async def handle_urgent_problem_description(message: types.Message, state: FSMContext):
    """Обработка срочного обращения - СТРОГИЙ ФИЛЬТР"""
    user_problem = message.text
    
    # Анализируем проблему через LLM
    analysis = await MockLLMService.analyze_problem(user_problem)
    
    # ЖЕСТКИЙ ФИЛЬТР: только high/critical priority
    if analysis['critical_level'] not in ['high', 'critical']:
        # Проверяем дополнительные ключевые слова для срочности
        urgent_keywords = ['полностью недоступен', 'остановка работы', 'не могу работать', 
                          'авария', 'не работает вся система', 'блокировка работы',
                          'финансовая ошибка', 'угроза безопасности', 'платеж не проходит',
                          'данные утеряны', 'система упала', 'критический сбой']
        
        problem_lower = user_problem.lower()
        has_urgent_keywords = any(keyword in problem_lower for keyword in urgent_keywords)
        
        if not has_urgent_keywords:
            await message.answer(
                ...
                "<b>Рекомендации:</b>\n"
                "1. Используйте обычное создание обращения через '📝 Создать обращение'\n"
                "2. Если проблема действительно срочная, укажите:\n"
                "   - 'Система полностью недоступна более 10 минут'\n"
                "   - 'Невозможно провести платежи/транзакции'\n"
                "   - 'Утечка конфиденциальных данных'\n\n"
                "<i>Срочные обращения проходят строгую проверку для эффективного распределения ресурсов поддержки.</i>",
                parse_mode="HTML"
            )
            await state.clear()
            return
        else:
            # Обновляем уровень критичности на основе ключевых слов
            analysis['critical_level'] = 'high'
    ...
    if current_state is None:
        ...
        for faq in MockDatabase.FREQUENT_QUESTIONS:
            ...
            if (any(keyword in user_text for keyword in faq_keywords) or 
                ...
                source_info = f"\n\n📊 <i>Встречаемость: {faq['frequency']}</i>"
                await message.answer(
                    faq["answer"] + source_info,
                    parse_mode="HTML",
                    reply_markup=get_feedback_keyboard()
                )
                await state.set_state(SupportStates.evaluating_solution)
        ...
        await message.answer(
            "🤔 <b>Кажется, я не совсем понял ваш вопрос.</b>\n\n"
            "Пожалуйста:\n"
...
async def cmd_stats(message: types.Message):
    ...
    escalated_tickets = len([t for t in MockTicketSystem._ticket_statuses.values() 
                           if "escalated" in t.get("status", "")])
    
    stats_text = f"""📊 <b>Статистика AI-агента поддержки (мок-данные)</b>

• Всего обращений: <b>{total_tickets}</b>
• Активных обращений: <b>{active_tickets}</b>
• Эскалировано: <b>{escalated_tickets}</b>

📈 <b>Распределение по приоритетам:</b>
• Критический: {len([t for t in MockTicketSystem._ticket_statuses.values() if t.get('priority') == 'Критический'])}
• Высокий: {len([t for t in MockTicketSystem._ticket_statuses.values() if t.get('priority') == 'Высокий'])}
• Средний: {len([t for t in MockTicketSystem._ticket_statuses.values() if t.get('priority') == 'Средний'])}

👥 <b>Пользователи:</b> {len(set(t.get('user_id') for t in MockTicketSystem._ticket_statuses.values() if t.get('user_id')))}
...
async def cmd_confidence_demo(message: types.Message):
    """Демонстрация уровня уверенности (скрытая команда для предзащиты)"""
...
📈 <b>Пример расчета для запроса "Не могу войти в систему отчетности":</b>
• Уверенность определения: <b>0.92</b> (высокая)
• Категория: "Проблемы с доступом"
• Источник: База знаний FAQ#1
• Решение: Предложено автоматически

📉 <b>Пример низкой уверенности для запроса "Что-то не так с программой":</b>
• Уверенность определения: <b>0.45</b> (низкая)
• Категория: "Общая техническая проблема"
• Решение: Создать обращение к специалисту
...
Каждый вопрос имеет ключевые слова для поиска и статистику встречаемости.
